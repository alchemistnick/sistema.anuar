...
# ==========================================
# FUNCIONES DE CONSULTA Y GESTIÓN EN FIRESTORE
# ==========================================

def obtener_modelos_activos():
    """Obtiene la lista de modelos ONU disponibles en la base de datos."""
    try:
        ...
        st.error(f"Error al conectar con Firestore (modelos): {e}")
        return []


def obtener_parametros_comites(id_modelo):
    """Obtiene los comités y cupos parametrizados para un modelo específico."""
    try:
...
def preinscribir_escuela(datos_escuela):
    """Registra una nueva institución/delegación en Firestore."""
    try:
        ...
        if doc_ref.get().exists:
            return False, f"El correo '{docente_email}' ya se encuentra preinscripto."
...
def validar_acceso_docente(email_doc, hash_ingresado):
    """Valida las credenciales de acceso del docente responsable."""
    try:
...
def obtener_bancas_asignadas(email_doc):
    """Obtiene las asignaciones de países y comités para la delegación."""
    try:
        email_clean = str(email_doc).strip().lower()
        docs = (
...
def guardar_participante_nomina(email_doc, dni, datos_participante):
    """Guarda o actualiza los datos de un estudiante en la nómina de la delegación."""
    try:
        email_clean = str(email_doc).strip().lower()
...
def registrar_pago_comprobante(email_doc, id_modelo, monto, drive_url):
    """Registra formalmente el comprobante de pago en la colección de pagos."""
    try:
...
def actualizar_estado_legajo(email_doc, estado):
    """Actualiza el estado general de la delegación (ej: CARGA_COMPLETA)."""
    try:
        email_clean = str(email_doc).strip().lower()
