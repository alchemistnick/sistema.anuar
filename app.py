...
# 🎛️ INTERRUPTOR DE MODO EVENTO (Cambiar a True durante los días del Modelo para bloquear inscripciones)
...
if MODO_SOLO_ACREDITACION and menu != "🎫 Acreditación Presencial":
    st.warning("⚠️ El sistema se encuentra en **Modo Acreditación**. Las inscripciones y cargas de documentación se encuentran temporalmente suspendidas.")
    st.stop()
...
if menu == "🎫 Acreditación Presencial":
    ...
    st.write("Ingrese el DNI del participante (estudiante o docente acompañante) para registrar su ingreso.")
                ...
                with st.spinner("Registrando institución..."):
                    try:
                        ...
                        if res.get("status") == "SUCCESS":
                            ...
                            st.success(f"¡Preinscripción exitosa! Tu Código de Delegación es: **{data_res.get('id_delegacion')}**.")
                        else:
    ...
    with st.form("form_login_escuela"):
        ...
        if btn_login:
            ...
            if escuela_encontrada:
                ...
                st.write(f"**Estado:** `{escuela_encontrada.get('estado', 'REGISTRADO')}`")
                ...
                st.error("Código o clave incorrectos.")
        ...
        if btn_subir_pago:
            if not id_del_pago or not hash_pago or not archivo_pago:
                ...
                payload = {
                    ...
                    "data": {"id_delegacion": id_del_pago, "secret_hash": hash_pago, "monto": monto_pago, "file_base64": file_b64, "file_name": archivo_pago.name, "file_mime": archivo_pago.type}
                }
                with st.spinner("Subiendo comprobante..."):
                    try:
                        res = requests.post(API_URL, json=payload).json()
                        if res.get("status") == "SUCCESS": st.success("¡Comprobante subido con éxito!")
                        else: st.error(f"Error: {res.get('message')}")
...
# ---------------------------------------------------------
    ...
    if id_del_nom and hash_nom:
        ...
        if not escuela:
            ...
            st.info(f"🏛️ **Institución:** {escuela.get('nombre_colegio')} | **Estudiantes:** {len(alumnos_actuales)}/{limite_bancas} | **Docentes:** {len(docentes_actuales)}/{int(escuela.get('docentes_acompanantes', 1))}")
            ...
            with tab_est:
                with st.form("form_agregar_estudiante"):
                    ...
                    rol_mnu = st.selectbox("Rol:", ["Delegado/a", "Embajador/a"])
                    nombre = st.text_input("Nombre:")
                    apellido = st.text_input("Apellido:")
                    dni = st.text_input("DNI:")
                    alergias_medicas = st.text_input("Alergias o Observaciones:", value="Ninguna")
                    file_ficha = st.file_uploader("Ficha Médica:", type=["pdf", "jpg", "png"], key="f_est")
                    file_aut = st.file_uploader("Autorización:", type=["pdf", "jpg", "png"], key="a_est")
                    ...
                    if st.form_submit_button("Guardar Estudiante"):
                        if not nombre or not apellido or not dni:
                            ...
                            payload = {
                                ...
                                "data": {
                                    "id_delegacion": id_del_nom, "secret_hash": hash_nom, "id_asignacion": id_asignacion_alum,
                                    "rol_mnu": rol_mnu, "nombre": nombre, "apellido": apellido, "dni": dni,
                                    "alergias_medicas": alergias_medicas, "ficha_b64": f_b64, "ficha_name": file_ficha.name if file_ficha else "", "ficha_mime": file_ficha.type if file_ficha else "",
                                    "aut_b64": a_b64, "aut_name": file_aut.name if file_aut else "", "aut_mime": file_aut.type if file_aut else ""
                                }
                            }
                            res = requests.post(API_URL, json=payload).json()
                            if res.get("status") == "SUCCESS": st.success("¡Estudiante guardado!"); st.rerun()
                ...
                with st.form("form_agregar_docente"):
                    ...
                    if st.form_submit_button("Guardar Docente"):
                        if not nombre_doc or not apellido_doc or not dni_doc or not email_doc:
                            st.error("Completa Nombre, Apellido, DNI y Email.")
                        else:
                            d_b64 = base64.b64encode(file_doc_aut.read()).decode('utf-8') if file_doc_aut else ""
                            payload_doc = {
                                "action": "GUARDAR_PARTICIPANTE_NOMINA",
                                "data": {
                                    "id_delegacion": id_del_nom, "secret_hash": hash_nom, "id_asignacion": "DOCENTE",
                                    "rol_mnu": "Docente Acompañante", "nombre": nombre_doc, "apellido": apellido_doc, "dni": dni_doc,
                                    "alergias_medicas": f"Email: {email_doc} | Cel: {cel_doc}",
                                    "ficha_b64": "", "ficha_name": "", "ficha_mime": "",
                                    "aut_b64": d_b64, "aut_name": file_doc_aut.name if file_doc_aut else "", "aut_mime": file_doc_aut.type if file_doc_aut else ""
                                }
                            }
                            res = requests.post(API_URL, json=payload_doc).json()
                            if res.get("status") == "SUCCESS": st.success("¡Docente guardado!"); st.rerun()
                            else: st.error(res.get('message'))
            ...
            if st.button("📌 Finalizar y Enviar Documentación a Revisión"):
                ...
                if res.get("status") == "SUCCESS": st.success("¡Enviado a revisión!")
                else: st.error(res.get('message'))
